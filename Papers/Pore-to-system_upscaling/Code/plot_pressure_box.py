...
relative_error_voxel = pd.read_csv("plot_P_relative_error_voxel.csv")
relative_error_marching_cubes = pd.read_csv("plot_P_relative_error_marching_cubes.csv")
relative_error_minkowski = pd.read_csv("plot_P_relative_error_minkowski.csv")
print(relative_error_minkowski)
...
#     ["Voxel Counting", "Marching Cubes", "Minkowski"],
# )
...
# 计算每组数据的均值（不需要标准差）
voxel_medians = np.median(data_voxel, axis=1)
marching_medians = np.median(data_marching_cubes, axis=1)
minkowski_medians = np.median(data_minkowski, axis=1)

# 设置x轴位置
positions = np.arange(len(Res))

# 绘制折线图（不带误差棒）
ax.plot(positions, voxel_medians, '-s', color='C2', label='Voxel Counting', alpha=0.7)
ax.plot(positions, marching_medians, '-o', color='C1', label='Marching Cubes', alpha=0.7)
ax.plot(positions, minkowski_medians, '-^', color='C0', label='Surface Density', alpha=0.7)
